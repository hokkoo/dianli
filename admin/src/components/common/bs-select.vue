...
export default {
  ...
  computed: {
    isShowPlus(){
      var status = false, searchValue = this.searchValue || '';
      _.each(this.options, (value) => {
        if(searchValue === (value.value || value.label || value)){
          status = true;
          return false;
        }
      });
  ...
  ready () {
    $(this.$els.select).blur(e => { this.show = false })
  },
  beforeDestroy () {
