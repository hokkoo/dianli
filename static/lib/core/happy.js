...
(function happyJS($) {
    ...
    $.fn.isHappy = function isHappy(config) {
        ...
        function defaultError(error) { //Default error template
            ...
            return $('<span id="' + error.id + '" class= "alert alert-danger ' + msgErrorClass + '" role="alert">' + error.message + '</span>');
        }
        ...
        function handleSubmit() {
            ...
            if (errors) {
                if (isFunction(config.unHappy)) return config.unHappy();
                return false;
        ...
        function bindInputChange() {
            fields.forEach(function(field) {
                $(field).on('input', field.testValid);
            });
        }
        ...
        function handleMouseDown() {
            ...
            //if(!pauseMessages) $(window).bind('mouseup', handleMouseUp);
            if (!pauseMessages) {
...
var happy = {
    ...
    date: function(val) {
       return (new Date(val)).toString().indexOf('Invalid')==-1;
    },
    ...
    phone: function(val) {
        return val && val.length == 11 && !isNaN(Number(val));
    },
    illegalS : function(val){
        return !/[@#\$%\^\<\>]+/g.test(val);
    },
